...
public abstract class Role
{
    ...
    public List<Tile> getNavigatableTiles(Player p){
        ...
        for (List<Tile> tl : ForbiddenIsland.getBoard().getBoard()){
            for (Tile t : tl){
                if (t != null && !t.isSunk() && ((Math.abs(t.getPositionX() - p.getPositionX()) <= 2.5 && t.getPositionY() == p.getPositionY()) || (Math.abs(t.getPositionY() - p.getPositionY()) <= 2.5 && t.getPositionX() == p.getPositionX())) && t.isMovable() && (t.getPositionX() != p.getPositionX() || t.getPositionY() != p.getPositionY())){
